...
const PhotoList = () => {
  ...
  console.log("🚀 -------------------------------------------------------------------🚀");
  console.log("🚀 ~ file: PhotoList.js ~ line 7 ~ PhotoList ~ cartItems", cartItems);
  console.log("🚀 -------------------------------------------------------------------🚀");
  return (
...
const PhotoItem = ({ info: { url, isFavorite, id } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <span className="absolute z-10 invisible opacity-0 cursor-pointer right-5 top-5 group-hover:opacity-100 group-hover:visible" 
      ...
      <button className="absolute invisible px-6 py-3 text-sm font-medium text-black transition-all bg-white rounded-lg opacity-0 bottom-5 left-2/4 -translate-x-2/4 group-hover:opacity-100 group-hover:visible"  onClick={() => addToCart({ url, isFavorite, id })}>
        Add to cart
      </button>
    </div>
  );
